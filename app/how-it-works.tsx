...
const parts: [string, string, string, string][] = [
  ["Launch", "Meteora Dynamic Bonding Curve", "Quick launch: name, ticker, image, go, on the default curve (opens at $5,000, graduates at $75,000, 1.25% fee, 0.5% to you). Advanced: five steps to pick the stock (S&P 500, Nasdaq 100, Tesla, NVIDIA, Microsoft, Amazon, Meta or McDonald's as xStocks, or pre-IPO Anthropic, OpenAI, SpaceX, Kalshi, Polymarket, Anduril, Figure AI or Neuralink from PreStocks), the curve shape (Classic, Steady, Rocket or Whale wall), the graduation target ($25K, $50K or $75K), the fee (1.25%, 2% or 3%), the extras and the fee model. Either way it is one wallet approval, with an optional dev buy of up to 75% of the supply in the same transaction that creates the pool, so nothing trades before you.", "Live on Devnet"],
  ["Fee model", "Sonata treasury program", "A 1.25% trading fee. Meteora takes 0.25% (0.05% of it comes back to Sonata as referrer on trades made here). Standard token: 0.5% to the creator (or to a fee module), 0.5% to Sonata. Reward token: the creator's 0.5% is paid to holders instead. Backed token: 0.25% to the creator and 0.25% into a stock reserve holders can cash out. No extra tax on transfers.", "Live on Devnet"],
  ...
  ["After graduation", "Meteora DAMM v2", `When a curve fills, the market moves to a DAMM v2 pool with its liquidity locked forever, split half to the creator and half to Sonata. The pool charges 1%, plus Meteora's volatility fee on fast moves. The creator claims their half's fees with one click.${PAYOUT_BOT_V2 ? " Sonata's half is collected every 15 minutes and split like the fees on the curve (for a Standard token, half to the fee model and half to Sonata), so creator payouts, holder rewards, fee modules and the Backed token's backing keep going after graduation." : ""}`, PAYOUT_BOT_V2 ? "Live on Devnet" : "Proven on Devnet"],
  ["Dollar targets", "Jupiter, Pyth and PreStocks", "Graduation targets are set in US dollars and converted at the price of the real tokenized stock on Solana. For xStocks such as SPYx or NVDAx that is the Solana market, and Pyth's equity feed is an independent check where our plan covers it: a gap above 1% blocks the launch. Pre-IPO tokens from PreStocks use their Solana market when it is deep enough and PreStocks' own mark price when it is not, and the launch page shows how far the market trades above or below that mark.", "Live on Devnet"],
