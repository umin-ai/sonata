...
import {isDeployableQuote,quoteAssetList} from '@/lib/treasury/quote-assets';
...
export function LaunchSettingsStep({step,value,onChange,price,pyth,onRefreshPrice}:{step:number;value:LaunchSettings;onChange:(s:LaunchSettings)=>void;price:number|null;pyth:PythState;onRefreshPrice:()=>void}){
 ...
 if(step===1)return <><p className="sr-note">Choose the stock token your community token trades against. Trading fees are collected in this asset.</p><div className="launch-options">{[...quoteAssetList.map(a=>[a.symbol,a.name]),['mNVDA','NVIDIA']].map(([symbol,name])=><button type="button" key={symbol} aria-pressed={value.quote===symbol} disabled={!isDeployableQuote(symbol)} onClick={()=>update({quote:symbol,...(symbol!==value.quote?{initial:2,target:12,pricing:undefined}:{})})}><TokenName symbol={symbol} size={32}/><span>{name}</span><small>{isDeployableQuote(symbol)?'Devnet launch available':'Coming soon · no Devnet token yet'}</small></button>)}</div></>;
 ...
 if(step===2){
  const why=pyth.status==='unconfigured'?`Dollar targets are unavailable, so these targets are set in ${value.quote}.`:pyth.status==='loading'?'Loading the stock price…':pyth.status==='error'||pyth.status==='unusable'?`Dollar targets are unavailable right now: ${pyth.message} These targets are set in ${value.quote} instead.`:'';
 ...
 return <><div className="floor-switch"><div><Label htmlFor="stock-floor"><ShieldCheck size={16}/> Stock Floor</Label><p className="sr-note">Half of net trading fees builds a floor. Any holder can burn their tokens for a share of it, paid in {value.quote}. You can never withdraw it. Only set at creation.</p></div><Switch id="stock-floor" checked={value.floor} onCheckedChange={(floor:boolean)=>update({floor,rewards:'treasury'})} aria-label="Stock Floor"/></div><div className="sr-detail-row"><span>Net trading fees</span><strong>{value.floor?'50% to your payout wallet · 50% to the Stock Floor':'50% to your payout wallet · 50% to your reserve, which you can withdraw'}</strong></div><p className="sr-note">Meteora keeps its protocol share first. Holder reward rounds can be added after launch from the Rewards page, on mSPY markets without a Stock Floor.</p></>;
}
