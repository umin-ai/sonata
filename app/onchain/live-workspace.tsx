"use client";
import { LaunchSettingsStep, initialSettings, canDeploy } from "@/app/launch-settings";
    ...
    <>
      ...
      <div className="launch-workspace">
        ...
        <Card className="sr-panel launch-form">
          ...
          {draft ? (
            ...
            <div className="space-y-4">
              ...
              {step >= 1 && step <= 3 && <LaunchSettingsStep step={step} value={settings} onChange={setSettings} price={price}/>}
              ...
              {step === 4 && <div className="launch-review"><TokenPair base={symbol} quote={settings.quote}/><div className="sr-detail-row"><span>Token name</span><strong>{name}</strong></div><div className="sr-detail-row"><span>Supply</span><strong>1,000,000,000</strong></div><div className="sr-detail-row"><span>Trading fee</span><strong>{settings.fee/100}%</strong></div><p className="sr-note">{deployable ? "Supported Devnet configuration." : "Preview configuration. Deployment is not supported for these settings yet."}</p><p className="sr-note">{deployable ? "Two transactions: launch the token, then activate the treasury. Review the estimated network cost before signing." : "Export these settings for review. No token or pool will be created."}</p></div>}
        ...
        <Card className="sr-panel launch-summary">
          ...
          {!draft && <><Badge variant="outline">{deployable ? "Devnet configuration" : "Preview · not deployable"}</Badge><div className="sr-detail-row"><span>Starting market cap</span><strong>{usdEstimate(settings.initial)}<small className="launch-quote-equivalent">{settings.initial} {settings.quote}</small></strong></div><div className="sr-detail-row"><span>Graduation market cap</span><strong>{usdEstimate(settings.target)}<small className="launch-quote-equivalent">{settings.target} {settings.quote}</small></strong></div><div className="sr-detail-row"><span>Quote reserve to graduate</span><strong>{curve ? (Number(curve.quoteThreshold)/1e8).toLocaleString(undefined,{maximumFractionDigits:8}) : "—"} {settings.quote}</strong></div>{curveError && <p role="alert" className="sr-note">{curveError}</p>}<p className="sr-note">Reserve calculated with Meteora’s SDK. Market cap is not the amount raised. USD estimates reference mainnet stock-token prices; mock tokens have no monetary value.</p>{referencePrice && <p className="sr-note">DEX Screener reference · {new Date(referencePrice.at).toLocaleTimeString()}</p>}</>}
        </Card>
      </div>
    </>
  );
