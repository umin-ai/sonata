...
const path = "artifacts/stockroom-liquidity-market.json";
const evidencePath = "artifacts/stockroom-liquidity-lifecycle.json";
...
async function send(label, tx, signers) {
  ...
  const signature = await sendAndConfirmTransaction(c, tx, signers, {
    commitment: "confirmed",
...
for (const [person, rawA, rawB, label] of [
  [alice, 3000000000000n, 1000000n, "fund-alice"],
  [bob, 5000000000000n, 1500000n, "fund-bob"],
  [trader, 2000000000000n, 1000000n, "fund-trader"],
]) {
...
if (!existsSync(path)) {
  const tokenAAmount = bn("20000000000000"),
    tokenBAmount = bn("4000000");
...
for (const [owner, nft, amount, label] of [
  [alice, aliceNft, "2000000000000", "deposit-alice"],
  [bob, bobNft, "4000000000000", "deposit-bob"],
]) {
  if (evidence.receipts.some((r) => r.label === label)) continue;
  const s = await state(),
