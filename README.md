...
- `/create` and `/onchain?pool=…`: Meteora DBC launch and Sonata treasury operations. Each launch creates its own DBC config. The launch is one page: only a name and ticker are required. It starts with the fee model; at the default 1.25% fee Meteora takes 0.25% of each trade (on trades built by the site it pays 0.05% of that back to Sonata as referrer) and Sonata gets 0.5%, and the rest goes by model:
