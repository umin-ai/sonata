...
A subsequent MockSPYx browser run completed all six actions with finalized receipts and verified zero final debt, collateral and lender shares. During the open loan, switching to MockNVDAx showed a separate empty position. Public browser evidence is in `evidence/mock-browser-lifecycle.json`.
